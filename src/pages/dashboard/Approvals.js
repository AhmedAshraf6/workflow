...
import ApproveRequestModal from '../../components/approveRequest/ApproveRequestModal';
...
export default function Approvals() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['get_Approve_requests'],
    queryFn: async () => {
      const data = await customFetch(
        `/ApplicationProcessInstanceSteps/ByStepTypeId/${1}`
      );
      return { data };
    },
    onError: (error) => {
